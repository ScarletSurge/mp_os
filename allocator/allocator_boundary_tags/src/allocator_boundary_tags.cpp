...
[[nodiscard]] void *allocator_boundary_tags::allocate(size_t value_size, size_t values_count)
{
...

    {
        ...
        else
        {
            while(current_block != nullptr)
            {
                ...
                {
                    start_of_memory = reinterpret_cast<unsigned char*>(previous_block) + get_metadata_size_of_block() + *reinterpret_cast<size_t*>(previous_block);
                }
            ...
            unsigned char* start_of_memory = reinterpret_cast<unsigned char*>(previous_block) + get_metadata_size_of_block() + *reinterpret_cast<size_t*>(previous_block);
            unsigned char* end_of_memory = reinterpret_cast<unsigned char*>(_trusted_memory) + get_ancillary_space_size() + get_full_size();
            size_t current_block_size = end_of_memory - start_of_memory;
            if (current_block_size >= requested_size &&
                (fit_mode == allocator_with_fit_mode::fit_mode::first_fit ||
    ...
    {
        size_t* size_of_block = reinterpret_cast<size_t*>(target_block);
        ...
        *size_of_block = requested_size - get_metadata_size_of_block();
        void** ptr_to_previous_block = reinterpret_cast<void**>(size_of_block + 1);
        void** ptr_to_next_block = reinterpret_cast<void**>(ptr_to_previous_block + 1);
        void** ptr_to_trusted_memory = reinterpret_cast<void**>(ptr_to_next_block + 1);

        *ptr_to_previous_block = nullptr;
        *ptr_to_next_block = nullptr;
    ...
    else
    {
        ...
        *ptr_to_previous_block = previous_to_target_block;
        *ptr_to_next_block = next_to_target_block;
        *ptr_to_trusted_memory = _trusted_memory;

        if(next_to_target_block != nullptr)
        {
            void** ptr_next_to_previous = reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(next_to_target_block) + sizeof(size_t));
            *ptr_to_next_block = target_block;
        }
        if(previous_to_target_block != nullptr)
        {
            void** ptr_previous_to_next = reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(previous_to_target_block) + sizeof(size_t) + sizeof(void*));
            *ptr_previous_to_next = target_block;
        }
    }
    *free_size -= requested_size;
    ...
    for(block_info value : information)
    {
        ...
        information_string += (is_occupied + " " + std::to_string(value.block_size) + " | ");
    }
    debug_with_guard(get_typename() + "END: allocation")->debug_with_guard("free size: " + std::to_string(*free_size));
...
void allocator_boundary_tags::deallocate(void *at)
{
    ...
    debug_with_guard(get_typename() + " END: deallocate")->debug_with_guard(" state blocks: " + data_str)->debug_with_guard(" free size: " + std::to_string(*free_size));
}
...
{
    *reinterpret_cast<allocator_with_fit_mode::fit_mode*>(reinterpret_cast<unsigned char*>(_trusted_memory) + sizeof(allocator*) + sizeof(logger*) + sizeof(size_t) + sizeof(size_t));
}
...
size_t allocator_boundary_tags::get_full_size() const noexcept
{
    *reinterpret_cast<size_t*>(reinterpret_cast<unsigned char*>(_trusted_memory) + sizeof(allocator*) + sizeof(logger*));
}

size_t allocator_boundary_tags::get_metadata_size_of_block() const noexcept
{
    return sizeof(size_t) + sizeof(void*) + sizeof(void*) + sizeof(void*);
}

void* allocator_boundary_tags::get_next_occupied_block(void *current_block) const noexcept
{
    *reinterpret_cast<void**>(reinterpret_cast<unsigned char*>(current_block) + sizeof(size_t) + sizeof(void*));
}
